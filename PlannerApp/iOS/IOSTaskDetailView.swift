...
struct IOSTaskDetailView: View {
    ...
    var body: some View {
        Form {
            ...
            Section("Даты") {
                IOSOptionalDatePicker(title: "Запланировано", date: $session.draft.scheduled)
                IOSOptionalDatePicker(title: "Срок", date: $session.draft.due)
            ...
            Section("Заметки") {
                TextEditor(text: $session.draft.notes).frame(minHeight: 120)
            }
